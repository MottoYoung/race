...
class AIPPT():
    ...
    def __init__(self, app_id, api_secret, Text, templateId): 
        self.APPid = app_id
    ...
    def create_task(self):
        ...
        formData = MultipartEncoder(
            fields={
                "query": self.text,
                "templateId": self.templateId,  # 使用传入的模板ID
                "author": "XXXX",
                "isCardNote": str(True),
                "search": str(False),
                "isFigure": str(True),
    ...
    #         "pageSize": size
    #     }
    def getTheme(self,num=1,size=10,style=None,color=None):
        url = "https://zwapi.xfyun.cn/api/ppt/v2/template/list"
        self.header = self.getHeaders()
        
        body = {
            "pageNum": num,  # 接收前端分页参数
            "pageSize": size,
        ...
        # 保持其他逻辑不变
        response = requests.request("GET", url=url, headers=self.header,json=body).text
        #print(response)
        print(num,"->num")
        return response
